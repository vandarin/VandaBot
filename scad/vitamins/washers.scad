...
module washer(type) {
    ...
    if(washer_soft(type))
        vitamin(str("WAR", hole * 10, diameter, thickness * 10, ": Rubber washer M", hole, " x ", diameter, "mm x ", thickness, "mm"));
    else
...
module star_washer(type) {
    ...
    inner = (hole / 2 + rad) / 2;
    spoke  = rad - hole / 2;
    vitamin(str("WAS", hole * 10, washer_diameter(type), thickness * 10,
