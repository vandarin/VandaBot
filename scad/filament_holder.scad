...
mounting_plate_thickness=35;
mounting_plate_width=min_spool_id+fastener_washer_clearance_radius;
mounting_plate_height=mounting_plate_thickness *1.1;
...
module mounting_plate(){
     $fn=fn_resolution;//smooth rounded rectangle
       ...
       translate( [0,(mounting_plate_width-max_filament_holder_od)/2,-(filament_holder_length+mounting_plate_thickness)/2 ])
                                        rotate([2,0,0])
                                        rotate([0,45,0])
        cube([19.392, 40, 19.392], center = true);
...
module spool_holder_stl(){
	...
	translate([0,0,max_filament_holder_od/2]){
		main_tube();
		rotate([90,0,0])
			translate([0,-0.5,-0.01]) //prevent intersecting faces
            difference() {
			mounting_plate();
                translate([0,-max_filament_holder_od/2,-70]){
...
module spool_holder_assembly(){
  vitamin("SPH: Spool Holder, credit http://www.thingiverse.com/thing:172484");
	main_tube();
	rotate([90,0,0]){
		mounting_plate();
